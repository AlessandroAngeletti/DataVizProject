...
```{r clean data 2, message = FALSE, warning = FALSE}
...
```{r clean data 3, echo = FALSE, message = FALSE, warning = FALSE}
...
selection_of_countries = c("US", "Israel", "Australia", "France", "Hungary")
...
```{r OLS Scores, message = FALSE, warning = FALSE}
dataAnalysis <- clean2 %>% 
  filter(price < 156,
...
```{r K-Fold Score, message = FALSE, warning = FALSE}
...
```{r LASSO Scores, message = FALSE, warning = FALSE}
set.seed(1234)
...
importance <- varImp(ols_model9, scale = TRUE)
plot(importance7)
```
...
```{r OLS Price, message = FALSE, warning = FALSE}
...
```{r K-Fold Price, message = FALSE, warning = FALSE}
control <- trainControl (
    method = "cv",
    number = 5,
    verboseIter = TRUE) #by setting this to true the model will report its progress after each estimation
...
```{r LASSO Price, message = FALSE, warning = FALSE }
set.seed(1234)
lambda_seq <- seq(0, 0.01, length = 1000)
