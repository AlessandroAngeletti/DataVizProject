...
ols_model5 <- lm(points ~ price + I(price^2) + country + year + as.factor(taster_name), dataAnalysis)
summary(ols_model5)
...
plsFit <- train(
  points ~ price + I(price^2) + country + year ,
  dataAnalysis,
  method = "lm",
  trControl = control
  )
...
model_lm <- lm(points ~ price + I(price^2) + country + year, training)
